...
void main() {
  ...
  group('Control Center Numismatic Audit Integration Tests', () {
    ...
    test('NumismaticMissingMagnitudesStrategy detects missing Motivo and repairs it correctly', () async {
      final species = await catalogRepo.getOrCreateSpecies('Moneda', type: 'Objeto');
      final sub = Subspecies(
        id: const Uuid().v4(),
        speciesId: species.id,
      ...
      final evalContext = AuditEvaluationContext(
        ...
        allInstanceMagnitudes: updatedInstance.magnitudes,
      );

      const strategy = NumismaticMissingMagnitudesStrategy();
      final cards = await strategy.evaluate(evalContext);

      expect(cards.length, equals(1));
      expect(cards.first.subtitle, contains('Motivo'));
    });
